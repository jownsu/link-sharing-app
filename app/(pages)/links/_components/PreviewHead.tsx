...
const PreviewHead = () => {
    ...
    return (
        <div className="absolute left-[50%] top-[6.3rem] flex h-[15.8rem] max-h-[15.8rem] w-full -translate-x-1/2 flex-col gap-[2rem] px-[1.2rem]">
            {!profile_picture && (
                <div className="bg-placeholder absolute left-[50%] top-0 size-[9.6rem] translate-x-[-50%] rounded-full"></div>
            )}

            {!first_name && !last_name ? (
                <div className="bg-placeholder absolute bottom-[2rem] left-[50%] h-[1.6rem] w-[16rem] translate-x-[-50%] rounded-[10.4rem]"></div>
            ) : (
            ...
            {!email ? (
                <div className="bg-placeholder absolute bottom-0 left-[50%] h-[.8rem] w-[7.2rem] translate-x-[-50%] rounded-[10.4rem]"></div>
            ) : (
